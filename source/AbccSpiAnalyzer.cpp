...
static t_ABCC_MOSI eMosiState = e_ABCC_MOSI_IDLE;
static t_ABCC_MISO eMisoState = e_ABCC_MISO_IDLE;
...
void SpiAnalyzer::AdvanceToActiveEnableEdgeWithCorrectClockPolarity()
{
	...
	for( ; ; )
	{
		/* If false, this function moves to the next active enable edge. */
...
bool SpiAnalyzer::GetWord(U64* plMosiData, U64* plMisoData, U64* plFirstSample)
{
	...
	*plFirstSample = mClock->GetSampleNumber();
	...
	for( U32 i=0; i<bits_per_transfer; i++ )
	{
		...
		{
			mCurrentSample = mClock->GetSampleNumber();
			PROCESS_DATA(mMosi,mosi_result,mMosiChannel);
			PROCESS_DATA(mMiso,miso_result,mMisoChannel);
			mArrowLocations.push_back( mCurrentSample );
		}

		/* If the trailing edge doesn't represent valid data, 
		** we want to allow the enable line to rise before the 
		** clock trailing edge and still report the frame */
		if( ( i == ( bits_per_transfer - 1 ) ) && ( mSettings->mDataValidEdge != AnalyzerEnums::TrailingEdge ) )
		{
			if( WouldAdvancingTheClockToggleEnable() == true )
			{
				/* Moving to the trailing edge would cause the clock to revert to inactive.  
				** Jump out, record the frame, and then move to the next active enable edge */
				need_reset = true;
				break;
			}

			/* Enable isn't going to go inactive, 
			** go ahead and advance the clock as usual.  
			** Then we're done, jump out and record the frame. */
			mClock->AdvanceToNextEdge();
			break;
		}
...
bool SpiAnalyzer::RunAbccMosiMsgSubStateMachine(bool fReset, bool* pfAddFrame, t_ABCC_MOSI *peMosiMsgSubState)
{
	static t_ABCC_MOSI eMosiMsgSubState = e_ABCC_MOSI_WR_MSG_SUBFIELD_size;
	static U8 bCnt = 0;
	if (fReset)
	{
...
void SpiAnalyzer::ProcessMisoFrame(t_ABCC_MISO eState, U64 lFrameData, S64 lFramesFirstSample)
{
	...
	** in the results for easy tracking of specific values */
	...
	{
		if(fMisoNewMsg) /* TODO: this marker should not be exclusive to "valid messages" */
		{
			fMisoNewMsg = false;
			if(mSettings->mEnableChannel != UNDEFINED_CHANNEL)
			{
	...
	if (eMosiState == e_ABCC_MISO_CRC32)
	{
		//TODO when an error in the statemachine occurs we should call mResults->CancelPacketAndStartNewPacket();
		mResults->CommitPacketAndStartNewPacket();
	}
	mResults->CommitResults();
}

void SpiAnalyzer::ProcessMosiFrame(t_ABCC_MOSI eState, U64 lFrameData, S64 lFramesFirstSample)
{
	...
	{
		if(bMosiLastToggleState == (U8)(lFrameData&0x80))
		{
			/* Retransmit event */
			result_frame.mFlags |= (SPI_PROTO_EVENT_FLAG);
		}
		...
		{
			bMosiLastToggleState = (U8)(lFrameData&0x80);
		}
	}
...
{
	t_ABCC_MOSI eMosiState_Current = e_ABCC_MOSI_IDLE;
	static t_ABCC_MOSI eMosiMsgSubState = e_ABCC_MOSI_WR_MSG_SUBFIELD_size;
	...
	** This would essentially indicate the begining of a new transaction. */
	...
	if(WouldAdvancingTheClockToggleEnable() == true)
	{
		if (eMosiState != e_ABCC_MOSI_IDLE)
		{
			...
			AddFragFrame(true,(U8)eMosiState_Current,lFirstSample,(S64)mClock->GetSampleNumber());
			eMosiState = e_ABCC_MOSI_IDLE;
			lFrameData = 0;
			dwByteCnt = 0;
			return true;
...
bool SpiAnalyzer::RunAbccMisoMsgSubStateMachine(bool fReset, bool* pfAddFrame, t_ABCC_MISO *peMisoMsgSubState)
{
	static t_ABCC_MISO eMisoMsgSubState = e_ABCC_MISO_RD_MSG_SUBFIELD_size;
	static U8 bCnt = 0;
	if (fReset)
	{
		/* Perform checks here that we were in the last state and that the 
...
{
	t_ABCC_MISO eMisoState_Current = e_ABCC_MISO_IDLE;
	static t_ABCC_MISO eMisoMsgSubState = e_ABCC_MISO_RD_MSG_SUBFIELD_size;
	bool fAddFrame = false;
	static U32 dwByteCnt = 0;
	static U64 lFrameData = 0;
	...
	** This would essentially indicate the begining of a new transaction. */
	...
	if(WouldAdvancingTheClockToggleEnable() == true)
	{
		...
		{
			/* We have a fragmented message */
			AddFragFrame(false,(U8)eMisoState_Current,lFirstSample,(S64)mClock->GetSampleNumber());
			eMisoState = e_ABCC_MISO_IDLE;
			lFrameData = 0;
			dwByteCnt = 0;
			return true;
